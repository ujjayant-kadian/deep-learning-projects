...
def cross_entropy(w, X, y):
    ...
    # p = [max(eps, min(1 - eps, x)) for x in p]
    ...
    #   cross_entropy += - (y[i] * math.log(p[i]) + (1 - y[i]) * math.log(1 - p[i]))
...
def predict_class(w, X, t):
    ...
    predicted_p = predict(w, X)
    predicted_class = (predicted_p >= t).astype(int)
...
def question_7():
    return {
      ...
      'Acc': 0.9555555555555556     # replace with the accuracy for that pair
      }    
