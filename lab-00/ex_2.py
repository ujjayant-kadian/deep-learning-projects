...
def find_root(f, df, x0):
  ...
  while True:
    ...
    if (derivative == 0):
      print("The derivative is 0, abandoning search!")
      return
    next_x = x - f(x)/df(x) #< compute using Newton-Raphson formula >
    print("{} - {}".format(i, x))
    if (abs(x - next_x) < 0.0001):#< insert expression to compute absolute difference of x and next_x > < 0.0001
      return x
    x = next_x# <update x>
    i += 1# <increment i>
    if (i > 100): #(< replace with expression checking number of iterations >):
